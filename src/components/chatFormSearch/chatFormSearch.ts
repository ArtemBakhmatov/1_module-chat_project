...
interface ChatFormSearchProps {
  [key: string]: unknown;
}

export default class ChatFormSearch extends Block {
  constructor(props: ChatFormSearchProps) {
    super({
      ...props,
    });
  }
  ...
  protected render(): string {
    return (
      `
        <form action="#" method="get" class="chatFormSearch">
          <svg 
            width="20" 
            height="20" 
            viewBox="0 0 20 20" 
          ...
          <input name="#" placeholder="Поиск" type="search" class="chatFormSearch__input">
        </form>
      `
    );
  }
